...
async function main() {
    ...
    console.log('Jan Rec Income Count:', data.results.length);

    const response2 = await fetch(`https://api.notion.com/v1/databases/${RECEPTION_DB}/query`, {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${NOTION_API_KEY}`,
            'Notion-Version': '2022-06-28', 'Content-Type': 'application/json'
    ...
    console.log('Feb Rec Income Count:', data2.results.length);
}
