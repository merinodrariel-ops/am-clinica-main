-- La caja física no puede depender de la fecha configurada en la computadora del operador.
CREATE OR REPLACE FUNCTION public.caja_fecha_operativa()
RETURNS DATE
LANGUAGE sql
STABLE
...
GRANT EXECUTE ON FUNCTION public.caja_fecha_operativa() TO authenticated;
...
BEGIN
    ...
    v_hoy_operativo := public.caja_fecha_operativa();
    IF p_fecha IS DISTINCT FROM v_hoy_operativo THEN
        RAISE EXCEPTION 'La apertura de caja debe ser para la fecha operativa % (Buenos Aires). Revisá la fecha y hora del equipo.', v_hoy_operativo;
    END IF;
    ...
    INSERT INTO caja_arqueos (
        ...
        jsonb_build_object(
            ...
            'fecha_operativa_servidor', v_hoy_operativo,
