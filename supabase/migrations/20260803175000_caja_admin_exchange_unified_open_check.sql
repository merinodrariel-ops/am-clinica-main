-- Currency exchanges must follow the unified physical cashbox once the branch
-- migrated away from separate reception/admin cash openings.
...
BEGIN
    ...
    IF v_unified_from IS NOT NULL AND p_fecha_movimiento >= v_unified_from THEN
        IF NOT EXISTS (
            SELECT 1
            ...
            RAISE EXCEPTION 'La caja fisica no esta abierta para esta fecha.';
        END IF;
    ELSIF NOT EXISTS (
        SELECT 1
