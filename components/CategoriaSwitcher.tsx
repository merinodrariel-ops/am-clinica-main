...
export default function CategoriaSwitcher() {
    const { isRealOwner, impersonatedCategoria, setImpersonatedCategoria, profile } = useAuth();
    ...
    const categorias = [
        ...
        { id: 'recaptacion', label: 'Recaptación', icon: UserCheck },
    ...
    if (isMinimized) {
        return (
            <button
                ...
                title="Cambiar de Categoría (Previsualización)"
            >
                <Zap size={24} fill={impersonatedCategoria ? "currentColor" : "none"} className={impersonatedCategoria ? "text-amber-400" : ""} />
        ...
        <div className="fixed bottom-4 right-4 z-[9999] flex flex-col items-end gap-2 animate-in slide-in-from-bottom-4 fade-in duration-300">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-blue-200 dark:border-blue-900 overflow-hidden transform transition-all overflow-y-auto max-h-[80vh]">
                ...
                <div className="p-3 grid grid-cols-1 gap-1">
                    ...
                    >
                        ...
                        Perfil Real: {profile?.categoria || 'Cargando...'}
                    </button>
                    ...
                    {categorias.map((cat) => (
                        <button
                            key={cat.id}
                            onClick={() => setImpersonatedCategoria(cat.id as any)}
                            className={clsx(
                                "flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-medium transition-all w-full text-left",
                                impersonatedCategoria === cat.id
                        ...
                        >
                            <cat.icon size={14} className={impersonatedCategoria === cat.id ? "text-amber-500" : "text-gray-400"} />
                            Ver como: {cat.label}
                        </button>
                    ))}
                </div>
            ...
            {impersonatedCategoria && (
                <div className="px-3 py-1 bg-amber-500 text-white text-[10px] font-bold rounded-full shadow-lg">
                    VISTA: {impersonatedCategoria.toUpperCase()}
                </div>
            )}
        </div>
    );
