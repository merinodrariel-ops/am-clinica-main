...
import { FileText, Download, CheckCircle, Clock, ChevronDown, ChevronUp, User, X, Eye } from 'lucide-react';
...
function PreviewModal({ worker, onClose, onDownloaded }: PreviewModalProps) {
        ...
        try {
            ...
            reader.onload = async () => {
                ...
                if (result.success && result.contrato) {
                    ...
                    toast.success('Contrato descargado y registrado');
                } else {
                    toast.warning('PDF descargado, pero no se pudo registrar: ' + (result.error || 'Error'));
                }
                onClose();
            };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#0f1623] border border-white/10 rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">

                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
                    <div className="flex items-center gap-2">
                        <Eye className="w-5 h-5 text-teal-400" />
                        <h2 className="text-base font-semibold text-white">Revisar antes de descargar</h2>
                    </div>
                ...
                <div className="px-6 py-5 space-y-4">
                    <p className="text-xs text-slate-400">
                        Revisá y corregí los datos del contrato. Podés editar cualquier campo antes de descargar.
                    </p>
                    ...
                    <div className="space-y-1">
                        ...
                        <select
                            value={form.anexoRol}
                            onChange={e => set('anexoRol', e.target.value as AnexoRol)}
                            className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-teal-500/50"
                        >
                    ...
                    {/* Preview summary */}
                    <div className="rounded-xl bg-white/5 border border-white/10 p-4 space-y-2 text-xs text-slate-300">
                        <p className="font-semibold text-white text-sm mb-1">Resumen del contrato</p>
                        ...
                        <p className="text-slate-500 pt-1">Incluye: Contrato maestro (12 cláusulas) + Anexo A (funciones del rol) + Anexo B (normas de convivencia)</p>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/10">
                    ...
                    >
                        ...
                        {downloading ? 'Generando...' : 'Descargar contrato'}
                    </button>
                </div>
            </div>
        </div>
