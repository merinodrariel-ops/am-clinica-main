...
type SyncPatient = Pick<
    Paciente,
    'id_paciente' | 'nombre' | 'apellido' | 'documento' | 'cuit' | 'fecha_nacimiento' | 'email' | 'direccion'
>;

export default function PresentacionesSyncTab({ initialPatientId }: PresentacionesSyncTabProps) {
    ...
    useEffect(() => {
        ...
        async function loadInitialPatient() {
            const { data } = await supabase
                .from('pacientes')
                .select('id_paciente, nombre, apellido, documento, cuit, fecha_nacimiento, email, direccion')
                .eq('id_paciente', initialPatientId)
                .eq('is_deleted', false)
                .single();

            if (!isMounted || !data) return;
            setSelectedPatient(data as SyncPatient);
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                setPatients((data || []) as SyncPatient[]);
            } catch (error) {
                console.error('Error searching patients for presentation sync:', error);
                setPatients([]);
            } finally {
            ...
            <div className="glass-card rounded-2xl p-5" style={{ background: 'hsla(230, 15%, 12%, 0.6)' }}>
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs mt-1" style={{ color: 'hsl(230 10% 45%)' }}>
                            Sincroniza archivos de presentación con la base de pacientes. Este módulo no genera contratos.
                        </p>
                    </div>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                    <span className="text-xs" style={{ color: 'hsl(230 10% 50%)' }}>
                        Sincronización global de presentaciones:
                    </span>
                    <select
                        value={syncAllLimit}
                ...
                {!selectedPatient && (
                    <div className="mt-4 relative">
                        <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'hsl(230 10% 45%)' }} />
                        <input
                            type="text"
                            value={searchQuery}
                ...
                {selectedPatient && (
                    ...
                    >
                        <div className="h-9 w-9 rounded-lg flex items-center justify-center" style={{ background: 'hsla(165, 100%, 42%, 0.16)' }}>
            ...
            {selectedPatient ? (
                        ...
                        <div className="rounded-xl p-3" style={{ background: 'hsla(230, 15%, 8%, 0.6)', border: '1px solid hsla(230, 15%, 20%, 0.8)' }}>
                            ...
                            {presentations.length === 0 ? (
                                ...
                                <div className="mt-2 space-y-1.5 max-h-40 overflow-y-auto pr-1">
                                    {presentations.slice(0, 12).map((item) => (
                                        <a
                                        ...
                                        >
                                            <span className="truncate max-w-[280px]">{item.drive_name}</span>
                ...
                <div className="rounded-2xl p-10 text-center" style={{ background: 'hsla(230, 15%, 12%, 0.45)', border: '1px dashed hsla(230, 15%, 24%, 0.9)' }}>
                    <p className="text-sm font-medium" style={{ color: 'hsl(230 10% 50%)' }}>
                        Selecciona un paciente para sincronizar sus presentaciones con base de datos.
                    </p>
                </div>
            )}
        </div>
