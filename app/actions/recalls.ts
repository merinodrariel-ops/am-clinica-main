...
export async function createRecallsFromAppointment(
    ...
    const { createRecallsForCompletedAppointment } = await import('@/lib/am-scheduler/recall-creation');
    return createRecallsForCompletedAppointment(
        appointmentId, appointmentType, patientId, appointmentDate, doctorId, await getUserEmail(),
    );
}
