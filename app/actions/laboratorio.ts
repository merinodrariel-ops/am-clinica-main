...
const LABORATORY_ORDER_CREATE_ROLES = new Set([
    'owner',
    'admin',
    'reception',
    'asistente',
...
export async function createLaboratoryOrderAction(input: {
    ...
    const { data: profile } = await supabase
        ...
        .eq('id', user.id)
        .maybeSingle();
    const role = normalizeCategoriaAlias(profile?.categoria || user.user_metadata?.categoria);
    if (!role || !LABORATORY_ORDER_CREATE_ROLES.has(role)) {
        return { ok: false, error: 'No tenés permisos para crear órdenes de laboratorio' };
    }

    // The legacy laboratorio_trabajos table has an incomplete/outdated RLS
    // policy in some environments. Keep the user/session check above, then
    // perform the bounded write with the server-only admin client.
    ...
    if (workflow?.id) {
        ...
        const { data: stages } = await db
            ...
            .eq('workflow_id', workflow.id)
            .order('order_index');
        const stage = (stages || []).find((item: { name: string }) => item.name.toLowerCase().includes('escaneo digital')) || stages?.[0];
        if (stage) {
            stageId = stage.id;
            ...
            if (existing?.id) {
                await db.from('patient_treatments').update({ metadata: { ...(existing.metadata || {}), laboratory_order_id: order.id } }).eq('id', existing.id);
            } else {
                const { data: treatment } = await db.from('patient_treatments').insert({
                ...
                if (treatment?.id) {
                    await db.from('treatment_history').insert({ treatment_id: treatment.id, new_stage_id: stage.id, comments: `Orden de laboratorio creada: ${order.id}` });
                }
            }
        }
    }
    ...
    if (emails.length > 0) {
        const { data: patient } = await db.from('pacientes').select('nombre, apellido').eq('id_paciente', input.patientId).maybeSingle();
