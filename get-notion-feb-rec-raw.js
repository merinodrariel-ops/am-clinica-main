...
// I'll just write one script that outputs Notion data in a format I can compare with SQL output.
...
async function main() {
    ...
    const notionData = rec.filter(p => {
        const cat = p.properties.Categoria?.select?.name;
        return ['Ingreso', 'Consulta', 'Control'].includes(cat);
    }).map(p => {
